...
public class dao {
    ...
    public static List<Products> searchProducts(String keyword) {
        // 4.1.5 Hệ thống tạo ra danh sách sản phẩm rỗng để lưu danh sách sản phẩm
        List<Products> list_products = new ArrayList<>();

        // 4.1.6 Hệ thống tạo câu lệnh truy vấn
        String sql = "SELECT * FROM product WHERE name LIKE ?";

        // 4.1.7 Hệ thống kết nối với cơ sở dữ liệu
        try (Connection connection = getConnection();
             // 4.1.8 Hệ thống tạo ra đối tượng PreparedStatement để thực thi câu lệnh
             PreparedStatement statement = connection.prepareStatement(sql)) {
            // 4.1.9 Hệ thống gán giá trị tham số (key word) cho câu lệnh SQL để tìm kiếm
            statement.setString(1, "%" + keyword + "%");
            // 4.1.10 Hệ thống tiến hành thực thi câu lệnh và lưu kết quả vào ResultSet
            ResultSet rs = statement.executeQuery();

            // 4.1.11 Hệ thống duyệt qua từng dòng kết quả
            while (rs.next()) {
                // 4.1.12 Hệ thống tạo đối tượng sản phẩm từ dữ liệu trong ResultSet
                Products product = new Products(
                        rs.getInt("p_id"),
                        rs.getString("name"),
                        rs.getInt("price"),
                ...
                // 4.1.13 Hệ thống thêm sản phẩm vào danh sách kết quả đã được tạo ở bước 4.1.5
                list_products.add(product);
            }
            // 4.2.8 Hệ thống không thể kết nối với cơ sở dữ liệu
        } catch (SQLException e) {
            // 4.2.9 Hệ thống in ra lỗi ở màn hình console
            e.printStackTrace();
        }
        // 4.1.14 Hệ thống trả về danh sách sản phẩm đã tìm được
